...

{
  ...
  vector<int> markerlist = {8,21,22,23,29,33,34,43};
  ...
  auto title2 = new TLegend(0.41+0.09,0.79,0.67+0.09,0.85); //(xmin, ymin, xmax, ymax) when moving, try to keep the intervals between min and max the same so the size of the title stays constant
  ...
  for (int i = 0 ; i < intersection_plot.size() ; i++)
	...
	draw_variable = "APL";
      ...
      plot -> GetYaxis() -> SetRangeUser(pow(10,3),pow(10,5)); //Y WINDOW SETTINGS
      plot -> GetXaxis() -> SetLimits(-50,3200); //X WINDOW SETTINGS
	...
	plot -> SetLineColor(kYellow-3);
	plot -> SetMarkerColor(kYellow-3);
	leg1[graph_number] -> SetTextColor(kYellow-3);
      }
  ...
  legend2title << "LO DY";//Legend 2 (right) title
  ...
  for (int i = 0 ; i < intersection_plot.size() ; i++)
	...
	leg2[graph_number_scaled] -> SetTextColor(kCyan+1);
      ...
      else if (graph_number_scaled == 7){
	plot -> SetLineColor(kYellow-3);
	plot -> SetMarkerColor(kYellow-3);
	leg2[graph_number_scaled] -> SetTextColor(kYellow-3);
      }
