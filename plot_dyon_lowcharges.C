...

{
  ...
  //FILENAMES
  string Cross_Section_Limits = "Cross_Section_Limits_lowcharges.csv"; //Declare Cross Section Limits file name that will be used for plots
  string Scaled_Cross_Sections = "Scaled_Cross_Sections_lowcharges.csv"; // Declare Scaled Cross Sections file name that will be used for plots
  
  string Intersection_File_Name = "Intersection_lowcharges.csv"; //Declare the name of the output file which contains the limit plot intersection points
  ...
  string col0, col1, col2, col3, col4, line;
  ...
  auto legend1 = new TLegend(0.64,0.18,0.81,0.46); //(xmin, ymin, xmax, ymax) try to keep the intervals between min and max constant

  ostringstream legend1title;
  legend1title << "95% CL Limits, " << gD[0] << " g_{D}"; //Change gD[0] if you need to manaully input the magnetic charge value for the legend title
  ...
  for (int i = 0 ; i < intersection_plot.size() ; i++)
	  ...
	  if (e[h] == get<2>(intersection_plot[i]) and gD[h] == get<1>(intersection_plot[i]) and spin[h] == get<0>(intersection_plot[i]) )
	    {
	      ...
	      if(e_scaled[h] == 150 and gD_scaled[h] == gD[0] and spin_scaled[h] == 1){
		if (GeV[h] == 3100 or GeV[h] == 3150 or GeV[h] == 3200){
		  tempvec.pop_back();
		  number_of_points -=1;
		}}
	      */ 
	...
	draw_variable = "APL";
      ...
      plot -> GetYaxis() -> SetRangeUser(1,pow(10,3)); //Y WINDOW SETTINGS
      plot -> GetXaxis() -> SetLimits(-200,6200); //X WINDOW SETTINGS
  ...
  auto legend2 = new TLegend(0.80, 0.18 , 0.92,0.46); //(xmin, ymin, xmax, ymax) try to keep interval consistent between max and min

  ostringstream legend2label;
  legend2label << "LO DY, " << gD[0] << " g_{D}"; //CHANGE gD[0] IF you need to manually change the magnetic charge in the legend title.
  ...
  for (int i = 0 ; i < intersection_plot.size() ; i++)
      ...
      for (int h = 0; h < number_of_points; h++)
	{
      ...
      plot -> GetXaxis() -> SetTitle("Mass [GeV]");
      plot -> GetYaxis() -> SetTitle("#sigma [fb]");
      plot -> GetYaxis() -> SetRangeUser(1,pow(10,3));
      plot -> GetXaxis() -> SetLimits(-200,6200);
  ...
  c1 ->SaveAs("limit_lowcharges.png");//File name of the plot
