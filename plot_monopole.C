...

{
  ...
  //Values
  float sqrt_s = 8; //Center of Mass Energy (TeV)
  float fb_inv = 2.21; //fb^{-1]
  ...
  for (int s = 0; s < possible_spin.size(); s++)
    {
      ...
      //looking for unique values in gD vector with speicfic spin values.
      ...
      for (int i = 0; i < row_total; i++)
	{
      ...
      sort(possible_gD.begin(),possible_gD.end()); //into order
      for (int i = 0; i < row_total_scaled; i++)
	{
	  if( std::find(possible_gD_scaled.begin(), possible_gD_scaled.end(), gD_scaled[i]) == possible_gD_scaled.end() and spin_scaled[i] == possible_spin[s] ) 
	    {
	      possible_gD_scaled.push_back(gD_scaled[i]);
	    }
	}
      sort(possible_gD_scaled.begin(),possible_gD_scaled.end()); // into order
      ...
      for (int g = 0; g < possible_gD.size(); g++)
	{
	  ...
	  for (int h = 0; h < row_total; h++)
	    { 
	      if (spin[h] == possible_spin[s] and gD[h] == possible_gD[g])
		{
	  ...
	  for (int h = 0; h < row_total_scaled; h++)
	    {
	      if (spin_scaled[h] == possible_spin[s] and gD_scaled[h] == possible_gD[g])
		{
		  ...
		  for (int j = 0 ; j<scaled.size()-1; j++)
		      ...
		      if  (x >= (scaled[j].first) and x >= (cross[i].first) and x <= (cross[i+1].first) and x <= (scaled[j+1].first))
			{
			  if (intersection_cout == 0){
			    gD_intersect.push_back(possible_gD[g]);
			    ...
			    x_str << x;
			    y_str << y;
			    intersection_info.push_back( make_tuple(possible_gD[g], x_str.str(), y_str.str())); // and store the x and y values into this variable
			  }
			  else
	...
	possible_gD = gD_intersect; //Replaced with only Intersected Plots
	...
	{
	  out << possible_spin[s] << "," << get<0>(intersection_info[k]) << "," << 0  <<"," << get<1>(intersection_info[k]) << "," << get<2>(intersection_info[k]) << "\n";
	}
      
      
      //Counting how many intersections
      mass_limits += possible_gD.size();
      sort(possible_gD.begin(), possible_gD.end());
      
      //Determine how many plots will be needed for each sub group of a given spin and gD
      double gD_plots_need = possible_gD.size() / 6.0;
      int tracker1 = 0;
      ...
      //divide e plots so its between 3-6 lines per plot.
      int t = 0; // index for while loop
      while (t < gD_plots_need)
	{
	  
	  int line_check = possible_gD.size() - tracker2;  //the little chunk helps seperate the plots so its always 3-6 lines per graph
	  if (line_check > 0)
	    {
	      if (line_check == 1)
		tracker2 -= 2;
	  ...
	  if (tracker2 > possible_gD.size())
	    tracker2 = possible_gD.size();
	  ...
	  for (int i = tracker1; i<tracker2; i++)
		...
		{
		  if (gD[j] == possible_gD[i] and spin[j] == possible_spin[s])
		    {
	  ...
	  double ymax_auto = *max_element(values.begin(),values.end())   * 5; //variable max value
	  ...
	  for (int i = tracker1; i<tracker2; i++)
		...
		draw_var = "APL";}
	      ...
	      g -> GetXaxis() -> SetLimits(-200 , 4000); //X WINDOW SETTINGS
	      
	      ostringstream label1;
	      label1 << (int)possible_gD[i] << " g_{D}";
	  ...
	  //plotting gD_scaled for a given spin
	  for (int i = tracker1; i<tracker2; i++)
	    {
		...
		{
		  if (gD_scaled[j] == possible_gD[i] and spin_scaled[j] == possible_spin[s])
		    {
	  ...
	  //Saving the plots, naming formate = spin_lowest-highest magnetic charge
	  ostringstream plotname;
	  plotname << possible_spin[s] << "_" << possible_gD[tracker1] << "gD-" << possible_gD[tracker2-1] << "gD.png";
