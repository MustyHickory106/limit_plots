...

{
  ...
  int row_total_scaled = 0; //variable used to count rows
  ...
  for (int s = 0; s < possible_spin.size(); s++)
    {     
      ...
      for (int i = 0; i < row_total; i++)
	{
	  if( std::find(possible_e.begin(), possible_e.end(), e[i]) == possible_e.end() and spin[i] == possible_spin[s] ) //true if not already seen
	    {
      ...
      for (int i = 0; i < row_total_scaled; i++)
	{
	  if( std::find(possible_e_scaled.begin(), possible_e_scaled.end(), e_scaled[i]) == possible_e_scaled.end() and spin_scaled[i] == possible_spin[s] ) 
	    {
      ...
      for (int g = 0; g < possible_e.size(); g++)
	{
	  ...
	  for (int h = 0; h < row_total; h++)
	    {
	      if (spin[h] == possible_spin[s] and e[h] == possible_e[g])
		{
	  ...
	  for (int h = 0; h < row_total_scaled; h++)
	    {
	      if (spin_scaled[h] == possible_spin[s] and e_scaled[h] == possible_e[g])
		{
	...
	{
	  out << possible_spin[s] << "," << "0" << "," << get<0>(intersection_info[k]) <<"," << get<1>(intersection_info[k]) << "," << get<2>(intersection_info[k]) << "\n";
	}
      
      //Counting how many intersections
      mass_limits += possible_e.size();
      sort(possible_e.begin(), possible_e.end());
      
      //Determine how many plots will be needed for each sub group of a given spin
      double e_plots_need = possible_e.size() / 6.0; // 6 for 6 per graph
      int tracker1 = 0;
      ...
      while (t < e_plots_need)
	{
	  ...
	  //Finding the larges and smallest values for windowing	 
	  vector<double> values;
	  for (int i = tracker1; i<tracker2; i++)
	    {
	      for (int j = 0; j < row_total; j++)
		{
		  if (e[j] == possible_e[i] and spin[j] == possible_spin[s])
		    {
	  ...
	  double ymax_auto = *max_element(values.begin(),values.end())    * 50; //variable max value
	  ...
	  for (int i = tracker1; i<tracker2; i++)
		...
		draw_var = "APL";}
	      ...
	      g -> GetXaxis() -> SetLimits(-200 , 4500); //X WINDOW SETTINGS
	  ...
	  //plotting e_scaled's for given spin
	  for (int i = tracker1; i<tracker2; i++)
	    {
		...
		{
		  if (e_scaled[j] == possible_e[i] and spin_scaled[j] == possible_spin[s])
		    {
	  ...
	  plotname << possible_spin[s] << "_" << 0 << "_" << possible_e[tracker1] << "e-" << possible_e[tracker2-1] << "e.png";
